document.addEventListener("DOMContentLoaded", function () {
  ...
  var marcadorOptions = {
      ...
      clickable: true,
      draggable: false,
      customIcon:"./img/transparente.jpg",
  };
  ...
  socket.on('jugadoresDatos', function (jugadores) {
      jugadores.forEach(jugador => {
          ...
          var marcadorNuevo = L.marker(jugador.ubicacion, markerOptions);
          var distance = map.distance(marcador.getLatLng(), marcadorNuevo.getLatLng());
          marcadorNuevo.bindPopup(`${jugador.nombre} está a ${distance.toFixed(2)} metros del centro`);
          marcadorNuevo.addTo(map);

          // Optional: You can store the marker objects for later use if needed
          marcadoresDinamicos.push(marcadorNuevo);
      });
  });
